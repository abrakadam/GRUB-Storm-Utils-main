...
static void on_refresh_clicked(GtkWidget *widget, gpointer data) {
    ...
    if (version_file.is_open()) {
        ...
        gtk_text_buffer_insert_at_cursor(buffer, "Ядро: ", -1);
        gtk_text_buffer_insert_at_cursor(buffer, version.c_str(), -1);
    ...
    if (os_release.is_open()) {
        ...
        while (std::getline(os_release, line)) {
            if (line.find("PRETTY_NAME") != std::string::npos) {
                ...
                gtk_text_buffer_insert_at_cursor(buffer, "ОС: ", -1);
                gtk_text_buffer_insert_at_cursor(buffer, distro.c_str(), -1);
                gtk_text_buffer_insert_at_cursor(buffer, "\n", -1);
                break;
            }
        }
...
static void on_grub_info_clicked(GtkWidget *widget, gpointer data) {
    ...
    if (grub_cfg.is_open()) {
        ...
        std::string count_str = "Количество записей: " + std::to_string(entry_count) + "\n";
        gtk_text_buffer_insert_at_cursor(buffer, count_str.c_str(), -1);
        grub_cfg.close();
    } else {
    ...
    if (pipe) {
        ...
        if (fgets(buffer_cmd, sizeof(buffer_cmd), pipe) != nullptr) {
            gtk_text_buffer_insert_at_cursor(buffer, "Версия GRUB: ", -1);
        ...
        while (std::getline(grub_cfg, line)) {
            if (line.find("menuentry") != std::string::npos) {
                ...
                if (start != std::string::npos && end != std::string::npos) {
                    ...
                    gtk_text_buffer_insert_at_cursor(buffer, entry.c_str(), -1);
                    entry_num++;
                }
            }
        }
...
int main(int argc, char **argv) {
    ...
    app = gtk_application_new("org.grubstorm.utils", G_APPLICATION_DEFAULT_FLAGS);
