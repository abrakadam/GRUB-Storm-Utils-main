...
struct SystemInfo {
    ...
    std::string kernel_version; // версия ядра linux
    std::string architecture;   // архитектура (x86, x86_64, arm и т.д.)
    std::string distribution;   // название дистрибутива
...
public:
    ...
    static std::string execute_command(const std::string& command);     // выполнить shell-команду и вернуть вывод
    static std::string get_distro_info();                               // определить дистрибутив linux
    static std::string get_cpu_info();                                  // информация о процессоре
    static std::string get_memory_info();                               // информация о памяти
    static bool is_64bit_system();                                      // проверка разрядности системы
